...
def generate_video(
    ...
    if lora_path:
        ...
        # LoRA effective scale must be lora_alpha/rank (== the training-time scale, 32/64 = 0.5
        # for this repo's config). The old value `1/lora_rank` (~0.008) under-applied the LoRA
        # by ~rank x, making it effectively a no-op.
    ...
    fused_state_dict = pipe.transformer.state_dict()
    
    # 比较融合前后的模型参数
    parameters_equal = True
    for key in original_state_dict:
        if not torch.equal(original_state_dict[key], fused_state_dict[key]):
            parameters_equal = False
            print(f"Difference detected in parameter: {key}")
            break

    if parameters_equal:
        print("All parameters are equal before and after fusing LoRA.")
    else:
        print("Parameters are different before and after fusing LoRA.")
