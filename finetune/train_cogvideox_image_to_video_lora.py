...
from data.bucket_sampler import (
    ASPECT_RATIO_512,
...
from data.dataset_image_video import ImageVideoDataset
...
def get_args():
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Run save checkpoint every X epochs. Validation consists of running the prompt `args.validation_prompt` multiple times: `args.num_validation_videos`."
        ),
    )
    parser.add_argument(
        "--guidance_scale",
    ...
    parser.add_argument(
        "--random_ratio_crop", action="store_true", help="Whether enable random adapt height and width in datasets."
    )
    parser.add_argument(
...
def main(args):
    ...
    # 假设 transformer 是你的模型实例
    

    # now we will add new LoRA weights to the attention layers
    # TODO 我需要修改这里的target module使得可以支持所有的linear层输入
    ...
    # linear_layers = get_linear_layers(transformer)
    ...
    # TODO 修改新的dataset类支持bucket的多桶训练
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                pixel_values = batch["pixel_values"].to(accelerator.device, dtype=vae.dtype) # [B F C H W]
                # print(f'pixel values shape{pixel_values.shape}')
                pixel_values = pixel_values.permute(0,2,1,3,4) # [B C F H W]
                image = pixel_values[:, :, :1].clone()

                image_noise_sigma = torch.normal(mean=-3.0, std=0.5, size=(1,), device=image.device)
                image_noise_sigma = torch.exp(image_noise_sigma).to(dtype=image.dtype)
                noisy_image = image + torch.randn_like(image) * image_noise_sigma[:, None, None, None, None]
                with torch.no_grad():
                    video_latent_dist = vae.encode(pixel_values).latent_dist
                    image_latent_dist = vae.encode(noisy_image).latent_dist
                video_latents = video_latent_dist.sample() * vae.config.scaling_factor
                ...
                if args.enable_text_encoder_in_dataloader:
                    prompt_embeds = batch['encoder_hidden_states'].to(device=video_latents.device)
                else:
                    with torch.no_grad():
                        prompt_ids = tokenizer(
                            batch["text"],
                            max_length=model_config.max_text_seq_length,
                            padding="max_length",
                            add_special_tokens=True,
                            truncation=True,
                            return_tensors="pt",
                        ...
                        prompt_embeds = text_encoder(
                            prompt_ids.input_ids.to(video_latents.device),
                            return_dict=False
                        )[0]
                ...
                def _prepare_rotary_positional_embeddings(
                    height: int, width: int, num_frames: int, device: torch.device
                ):
                    vae_scale_factor_spatial = 2 ** (len(vae.config.block_out_channels) - 1) if vae is not None else 8

                    p = unwrap_model(transformer).config.patch_size
                    p_t = unwrap_model(transformer).config.patch_size_t

                    grid_height = height // (vae_scale_factor_spatial * p)
                    grid_width = width // (vae_scale_factor_spatial * p)
                    base_size_height = unwrap_model(transformer).config.sample_height // p
                    base_size_width = unwrap_model(transformer).config.sample_width // p
                    ...
                    if p_t is None:
                        ...
                        freqs_cos, freqs_sin = get_3d_rotary_pos_embed(
                            embed_dim=unwrap_model(transformer).config.attention_head_dim,
                ...
                image_rotary_emb = (
                    _prepare_rotary_positional_embeddings(height, width, video_latents.size(1), video_latents.device)
                    if unwrap_model(transformer).config.use_rotary_positional_embeddings
                    else None
                )
                # print(f'check timesteps{timesteps}')
                # print(f'len time steps shape{len(timesteps.shape)}')
                # import pdb
                # pdb.set_trace()
                ...
                model_output = transformer(
                    ...
                    timestep=timesteps,
                    ofs = torch.tensor([2.]).to(dtype=noisy_model_input.dtype, device=noisy_model_input.device),
